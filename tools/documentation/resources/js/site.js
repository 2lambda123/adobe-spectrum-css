...
window.addEventListener("DOMContentLoaded", function () {
  ...
  function selectNavItem(href) {
    ...
    if (
      navLink &&
      navLink.parentNode.classList.contains("spectrum-SideNav-item")
    ) {
  ...
  function loadPage(href) {
    ...
    function handleLoad() {
      ...
      function handleLinkOnload(dependencyName, event) {
        ...
        loadingDependencies.splice(
          loadingDependencies.indexOf(dependencyName),
          1
        );
      ...
      function changeContent() {
        ...
        document.querySelector("title").innerHTML =
          template.content.querySelector("title").innerHTML;

        // Change content
        var oldMainContainer = document.querySelector(
          ".spectrum-Site-mainContainer"
        );
        oldMainContainer.parentNode.insertBefore(
          newMainContainer,
          oldMainContainer
        );
      ...
      if (req.status === 200) {
        ...
        newMainContainer = template.content.querySelector(
          ".spectrum-Site-mainContainer"
        );

        if (newMainContainer) {
          // Load in extra deps before last link
          var currentDependenciesNodeList =
            document.querySelectorAll("[data-dependency]");
          var currentDependencies = Array.prototype.slice
            .call(currentDependenciesNodeList)
            .map(function (link) {
          ...
          if (newDependencies.length) {
            var loadingDependencies = Array.prototype.slice
              .call(newDependencies)
              .map(function (link) {
                return link.getAttribute("data-dependency");
              });
            ...
            console.log(
              "  ⏳ Loading " +
                loadingDependencies.length +
                " new dependencies..."
            );

            var beforeLink =
              currentDependenciesNodeList[
                currentDependenciesNodeList.length - 1
              ];
            newDependencies.forEach(function (link) {
              link.onload = handleLinkOnload.bind(
                link,
                link.getAttribute("data-dependency")
              );
          ...
          console.error(
            "Could not find main container within loaded HTML file"
          );
          handleError(
            "Content not found",
            "The page loaded successfully, but doesn't contain documentation content.",
            204
          );
        }
      } else {
...
<div class="spectrum-IllustratedMessage">\
  ...
  <h2 class="spectrum-Heading spectrum-Heading--pageTitle spectrum-IllustratedMessage-heading">' +
        title +
        '</h2>\
