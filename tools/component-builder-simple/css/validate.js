...
const getTokensUsedInCSS = (root, componentTokens, coreTokens) => {
     ...
     */
    const getTokensUsedInValueNode = (node, {
      usedTokens = new Set(),
    } = {}) => {
        ...
        parse(decl.value)?.nodes.forEach(node => {
          getTokensUsedInValueNode(node).forEach(tokenName => {
            if (coreTokens.size > 0 && coreTokens.has(tokenName)) {
              coreTokensUsed.set(tokenName, (coreTokensUsed.get(tokenName) || 0) + 1);
            } else if (componentTokens.has(tokenName)) {
              componentTokensUsed.set(tokenName, (componentTokensUsed.get(tokenName) || 0) + 1);
            }
  ...
  const getCoreTokens = (tokenPackageName, { verbose = true }) => {
    ...
    if (verbose) {
      const { version } = require(`${tokenPackageName}/package.json`, fetchOptions);
      console.log(`  ${chalk.bold.gray('⟫')} using: ${tokenPackageName}@${chalk.bold(`${version}`)}`);
    }
 ...
 **/
module.exports = async (css, coreTokenPackage, { componentName, ignorePrefixes, verbose = true }) => {
    if (!css) return;
    ...
    for (const tokenName of usedTokens) {
      if (
        ...
        errors.push(`${chalk.yellow(componentName)} uses undefined token ${tokenName}.`);
      }
    }
    ...
    if (!errors.length) {
        if (verbose) {
          console.log(`  ${chalk.bold.gray('⟫')} using: ${chalk.bold(`${usedTokens.size}`)} tokens`);
        }
        return Promise.resolve();
    }

    if (verbose) {
        console.log(`  ${chalk.yellow(componentName)} | ${chalk.bold(`${errors.length}`)} errors:`);
