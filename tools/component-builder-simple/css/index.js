...
function getTokensUsedInValueNode(node, usedTokens) {
  ...
  if (!node.nodes) return usedTokens;
...
function getTokensUsedInCSS(root, coreTokens, componentTokens) {
  ...
  root.walkRules((rule, _ruleIndex) => {
    rule.walkDecls((decl) => {
      ...
      for (const node of parse(decl.value)?.nodes) {
        const usedTokensInValue = getTokensUsedInValueNode(node);
        if (!usedTokensInValue.length) continue;

        for (const tokenName of usedTokensInValue) {
...
async function getCoreTokens() {
  ...
  if (coreTokensPkg && verbose) console.log(
    "Core tokens version:",
    await fsp.readFile(coreTokensPkg, "utf8").then(JSON.parse).then((pkg) => pkg.version)
  );
  const contents = await fsp.readFile(coreTokensFile, "utf8");
  const root = postcssReal.parse(contents);
...
async function buildCSS() {
  ...
  await fsp.writeFile("dist/index.css", processedCss.css)
...
async function buildCSSWithoutThemes() {
  ...
  try {
    ...
    const isThemesFolderExists = await fsp.stat("themes").catch(() => false);
    ...
    const cssContents = await Promise.all(files.map(file => fsp.readFile(file, "utf-8")));
    const css = cssContents.join("\n");
    ...
    await fsp.mkdir(path.join("dist/themes"), { recursive: true });

    // Write the processed CSS to a file
    await fsp.writeFile("dist/index-base.css", processedCss.css);
  } catch (e) {
    console.error(e);
  }
}
...
async function buildCSSThemeIndex() {
  ...
  try {
    const isThemesFolderExists = await fsp
      .stat("themes")
      .catch(() => false)
    if (isThemesFolderExists) {
      ...
      // Read the contents of all CSS files
      const cssContents = await Promise.all(
        allCss.map((file) => fsp.readFile(file, "utf-8"))
      )
      ...
      // Write the processed CSS to a file
      await fsp.writeFile("dist/index-theme.css", processedCss.css)
    }
  } catch (e) {
    console.error(e)
  }
...
async function buildCSSThemes() {
  ...
  try {
    const isThemesFolderExists = await fsp
      ...
      .catch(() => false)
    if (isThemesFolderExists) {
      const cssFiles = await fsp.readdir("themes/")
...
async function buildExpressTheme() {
  try {
    ...
    const css = await fsp.readFile("dist/index-theme.css", "utf-8")
    ...
    await fsp.writeFile("dist/themes/express.css", processedCss.css)
  } catch (e) {
    console.error(e)
  }
}
...
async function checkCSS(globFile) {
  ...
  try {
    ...
    const cssContents = await Promise.all(
      cssFiles.map((file) => fsp.readFile(file, "utf-8"))
    )
