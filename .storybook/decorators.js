...
export const withContextWrapper = makeDecorator({
    ...
    wrapper: (StoryFn, { args, parameters, globals, viewMode }) => {
        ...
        const staticVariant = args.variant;
        ...
        useEffect(() => {
            ...
            els.forEach((root) => {
                ...
                if (staticColor || staticVariant) {
                    if (staticColor === "white" || staticVariant === "light") {
                        root.style.backgroundColor = "rgba(15, 121, 125)";
                    } else if (staticColor === "black" || staticVariant === "dark") {
                        root.style.backgroundColor = "rgba(181, 209, 211)";
                    }
                }

                root.querySelectorAll('[data-story-block="true"]').forEach((el) => {
                    el.style.overflow = "visible";
                });
            });
        }, [context, scale, color, staticColor, staticVariant]);
