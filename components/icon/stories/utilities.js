...
export const fetchIconSVG = ({ iconName, setName = "workflow" }) => {
    if (!iconName) return;
    ...
    if (setName === "workflow") {
        try {
            icon = require(
                `@adobe/spectrum-css-workflow-icons/dist/${scale !== "medium" ? `24` : `18`}/${iconName}.svg?raw`,
            );
    ...
    try {
        icon = require(`@adobe/spectrum-css-ui-icons/${scale ? scale : "medium"}/${iconName}.svg?raw`);
        if (icon) return icon.default ?? icon;
    } catch (e) {
        /* ignore */
    }
