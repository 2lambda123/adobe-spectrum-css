...
export const Template = ({
    ...
    const rampSVG = html` <svg viewBox="0 0 240 16" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        <path
                            ...
                            ${when(
                                ...
                                () =>
                                    html` <div
                                        ...
                                        role="textbox"
                                        aria-readonly="true"
                                        aria-labelledby=${ifDefined(id && label ? `${id}-label` : undefined)}
                                    >
            ...
            <div class="${rootClass}-controls" role=${ifDefined(isRamp ? "presentation" : undefined)}>
                ${values.map((value, idx) => {
                    ...
                    return [
                        ...
                        when(isFirst && isRamp, () => html`<div class="${rootClass}-ramp">${rampSVG}</div>`),
                        when(isFirst && showTicks && !isRamp, () => renderTick({ from: min, to: max })),
                        renderHandle({ position: getPosition(value), value, idx }),
                        when(isLast && !isRamp, () => renderTrack({ width: getWidth(value, max) })),
                        when(
                            ...
                            () =>
                                html` <div
                                    ...
                                    style=${ifDefined(
                                        styleMap({
                                            [rtl ? "right" : "left"]: `${
                                                value > centerPoint ? getPosition(centerPoint) : getPosition(value)
                                            }%`,
