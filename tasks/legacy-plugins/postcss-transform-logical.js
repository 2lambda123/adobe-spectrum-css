...
module.exports = postcss.plugin("postcss-transform-logical", function (opts) {
	...
	return function (root) {
		root.walkDecls(/transform/, (decl) => {
			let value = parse(decl.value);
			if (
				!value ||
				value.nodes.length === 0 ||
				value.nodes[0]?.value !== "logical"
			) return;
			...
			value.walkFuncs((node) => {
				if (node.name === "rotate") {
					originalRotation = node.nodes[0].value;
					rotationNode = node;
				} else if (node.name === "matrix") {
					throw new Error(
						"postcss-transform-logical: logical flips cannot be performed on transforms that use matrix()"
					);
				}
			});
			...
			if (rotationNode !== null) {
				...
				if (parseInt(originalRotation, 10) !== 0) {
					...
					let rtlRule = postcss.parse(
						`${newSelector} { transform: ${matrix} ${value}; }`
					);
					root.insertBefore(rule, rtlRule);
				}
