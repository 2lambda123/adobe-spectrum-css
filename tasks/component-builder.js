...
function getAllVars() {
	...
	return files.reduce((variableList, file) => {
...
async function processCSS(from, content, { configPath } = {}) {
	return postcssrc({
		from,
	}, configPath)
 ...
 */
async function legacy_bakeVars(content, packagePath = process.cwd()) {
	if (!content || content.length === 0) return;
	...
	const usedVars = variableList.reduce((usedVars, varName) => {
		if (
			!varName.startsWith("--spectrum-global") &&
			!varName.startsWith("--mod") &&
			!varName.startsWith("--highcontrast") &&
			allVars[varName] &&
			allVars[varName] !== undefined &&
			typeof allVars[varName] !== "undefined"
		)
	...
	const contents = classNames && usedVars && Object.keys(usedVars).length > 0 ? `
...
async function legacy_buildIndexVars(packagePath = process.cwd()) {
	...
	const result = await processCSS(path.join(packagePath, files[0]), contents, {
		configPath: path.join(__dirname, "../postcss.legacy.config.js"),
	});
...
const legacy_build = async (packagePath = process.cwd()) => {
	return legacy_buildIndexVars(packagePath).then((content) => {
		if (!content || content.length === 0) return;
		return legacy_bakeVars(content, packagePath);
	});
};
