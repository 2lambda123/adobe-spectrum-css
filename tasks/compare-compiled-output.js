 ...
 * @param {string} package - The name of the package to process (required)
...
async function processPackage(packageName, { cwd } = {}) {
  ...
  const pkgPath =
    ...
    join(cwd, packageName, "package.json") ?? join(pathing.components, packageName, "package.json");
  const pkg =
      ...
      if (existsSync(tarballPath)) {
        await tar
          .extract({
            ...
            filter: (path) => path.startsWith("package/dist"), //@todo is this the right path?
            strip: 2,
          })
