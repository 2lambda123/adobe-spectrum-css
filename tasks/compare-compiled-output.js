...
const log = {
    ...
    writeTable: (data, padding = 30) => {
        ...
        const table = (data = [], size = padding) => data.map((row) => `${row ?? " "}`.padEnd(size)).join("");
...
async function renderTemplate(templateFile, templateVariables = {}, outputPath = undefined) {
    templateFile = join(__dirname, "templates", `${templateFile}.njk`);

    if (typeof templateFile === "undefined" || !existsSync(templateFile)) {
        Promise.reject(`Template file ${templateFile} not found. Could not render output.`);
        return;
    }
...
async function generateDiff(filepath, outputPath, localContent, remoteContent, renderData) {
...
async function processComponent(component, { cwd = process.cwd(), output, cacheLocation }) {
    if (!component) {
        return Promise.reject("No component specified.");
    }

    cleanAndMkdir(join(output, "diffs", component));

    const pkg = require(join(cwd, component, "package.json")) ?? {
    ...
    if (existsSync(join(cwd, component))) {
        ...
        const files = (await fg("**/*.css", { cwd: join(cwd, component, "dist") })) ?? [];
        files.forEach((file) => filelist.add(file));
    } else {
        warnings.push(`${`${relative(root, join(cwd, component))}`.brightYellow} not found locally.\n`);
    }
    ...
    if (pkg && pkg.name) {
        ...
        if (tag) {
            ...
            if (!existsSync(tarballPath) && tarballUrl) {
                ...
                if (!tarballFile || (tarballFile.status && tarballFile.status !== 200)) {
                    log.error(`Failed to fetch release content for ${pkg.name}`);
                } else {
                    await writeFile(tarballPath, await tarballFile.buffer(), { encoding: "utf-8" });
                }
            }

            // The tarball path should exist locally now; if not, something went wrong
            if (existsSync(tarballPath)) {
                // Extract the tarball into a clean directory
    ...
    if (found < 1) {
        return Promise.reject(`${component.cyan} does not exist. Try checking the package's name and spelling.`);
    }

    if (filelist.size === 0) {
        return Promise.reject(`No compiled assets found associated with ${component.cyan}.`);
    }

    // For all files found locally & on npm, report back on it's sizes
    return Promise.all(
        [...filelist].map(async (filename) =>
            processFile(filename, join(cwd, component, "dist"), join(output, "latest")),
        ),
    ).then((results) => {
...
async function processFile(filename, localPath, comparePath) {
    ...
    if (existsSync(join(localPath, filename))) {
        const content = await readFile(join(localPath, filename), { encoding: "utf-8" });
    ...
    if (existsSync(join(comparePath, filename))) {
        const content = await readFile(join(comparePath, filename), { encoding: "utf-8" });
...
async function main(components = allComponents, output, { skipCache = false } = {}) {
    /** Setup the folder structure */
    cleanAndMkdir(output);

    // This is our cache of fetched tarballs from npm; do not clean this directory
    // unless we want to re-fetch the tarballs
    cleanAndMkdir(join(output, "packages"), skipCache);
    ...
    const results = await Promise.all(
        components.map((component) => {
            return processComponent(component, {
                ...
                output,
                cacheLocation: join(output, "packages"),
            });
        }),
    ).catch((err) => {
        log.error(err);
    ...
    for (const { component, warnings, tag, pkg, fileMap } of results) {
        const files = [...fileMap.keys()];

        if (!files || files.length === 0) {
            log.error(`No compiled assets found associated with ${`@spectrum-css/${component}`.brightYellow}.`);
            continue;
        }
        ...
        files.forEach(async (file) => {
            ...
            if (local && local.content && npm && npm.content) {
                promises.push(
                    generateDiff(file, join(output, "diffs", component), local.content, npm.content, {
                        component,
                        tag,
                    }),
                );
...
let { _: components = [], output = join(root, ".diff-output"), cache = true } = yargs(hideBin(process.argv)).argv;

main(components, output, { skipCache: !cache });
