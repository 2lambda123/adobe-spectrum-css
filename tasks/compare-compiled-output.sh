...
# Allow nvm to be used in the script
unset PREFIX;
...
commit_compiled_assets() {
    ...
    # Ensure that the dependencies are installed before compiling the assets
    report "yarn install --silent --frozen-lockfile --non-interactive"

    # Compile the assets
    report "yarn --silent builder tag:component,ui-icons --skip-nx-cache"
    ...
    if [[ -z "$HAS_CHANGES" ]]; then
        ...
        report "git commit -m \"chore: refresh compiled output for comparisons\" --quiet --porcelain"

        # Capture the commit hash for the comparison URL
        COMMIT_HASH=$(git rev-parse HEAD)

        # Echo the commit hash to the console
        printf "\nCommit hash: %b\n" "\e[35m${COMMIT_HASH}\e[0m"

        # Push the branch to GitHub and open a comparison
        report "git push origin ${COMPARE_BRANCH} --quiet --porcelain"
    fi
...
clean_up_branch() {
    ...
    report "git push origin --delete ${TEMP_BRANCH_NAME} --porcelain"
}
...
main() {
    ...
    echo "BRANCH_COMMIT_HASH: $BRANCH_COMMIT_HASH"

    # Confirm that the commit hashes are not empty
    if [[ -n "$BASE_COMMIT_HASH" ]] && [[ -n "$BRANCH_COMMIT_HASH" ]]; then
