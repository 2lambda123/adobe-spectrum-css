		...
		files: [
			{
				...
				fn: async (event, file) => {
					if (!["add", "change"].includes(event)) return bs.reload();
			...
			},
			{
				...
				fn: async (event, file) => {
					if (!["add", "change"].includes(event)) return bs.reload();
					return watch_Styles(path.join(dirs.components, file)).then(() => bs.reload());
				},
			},
			{
				match: ["*/metadata/*.yml"],
				...
				fn: async (event, file) => {
					...
					const componentName = getPackageFromPath(path.join(dirs.components, file));
					if (!componentName) return bs.reload();

					const globalData = await fetchData_forGlobal();
					return build_forPackage(path.join(dirs.components, componentName), globalData).then(() => bs.reload());
				},
			},
			{
				...
				fn: async (event) => {
					...
					const globalData = await fetchData_forGlobal();
					return build_forSite(globalData).then(() => bs.reload());
				},
			},
			{
				match: ["resources/**"],
				...
				fn: async (event) => {
					...
					return copy_Resources().then(() => bs.reload());
				},
			},
			{
				match: ["**"],
