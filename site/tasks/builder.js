...
function getDependencies({
	...
	for (const dep of [
		...
		if (dep.includes("builder")) continue;
		deps.add(dep);
	}
 ...
 * @returns {Promise<void>}
 */
async function copyPackageAssets(sourcePath) {
 ...
 * @returns {Promise<void>}
 */
async function copyWorkflowIcons() {
	const resource = require.resolve("@adobe/spectrum-css-workflow-icons/dist/spectrum-icons.svg", {
		paths: [rootPath, process.cwd()]
	});

	if (!resource) {
		console.warn(`🔴 Unable to locate ${"@adobe/spectrum-css-workflow-icons".yellow} icons package.`);
		return;
	}
	return safeCopy(
		resource,
...
async function copyUIIcons() {
	...
	if (!resource) {
		console.warn(`🔴 Unable to locate ${"@spectrum-css/ui-icons".yellow} icons package.`);
		return;
	}
...
async function copyVars() {
	...
	if (!resourcePath) {
		console.warn(`🔴 Unable to locate ${"@spectrum-css/vars".yellow} package.`);
		return;
	}
 ...
 * @returns {Promise<void>}
 */
async function copyExpressVars() {
	const resourcePath = path.dirname(require.resolve("@spectrum-css/expressvars/package.json", {
		paths: [rootPath, process.cwd()]
	}));

	if (!resourcePath) {
		console.warn(`🔴 Unable to locate ${"@spectrum-css/expressvars".yellow} package.`);
		return;
	}
 ...
 * @returns {Promise<void>}
 */
async function copySiteDependencies() {
	return Promise.all([
		copyResources(),
		copyCoreTokens(),
		copyVars(),
 ...
 */
async function getDataForDependency(depName) {
 ...
 */
async function getDataForIndexes() {
	const folders = await fg("*", {
		cwd: path.join(rootPath, "components"),
 ...
 * @returns {Promise<void>}
 */
async function getDataForNavigation() {
	const folders = await fg("*", {
		cwd: path.join(rootPath, "components"),
		onlyDirectories: true,
	});
 ...
 */
async function buildPagesForDependency(depName) {
...
async function buildAllPackages() {
	...
	return Promise.all(dependencies.map((dep) => buildPagesForDependency(dep))).then(data => data.flat());
}
...
async function buildAllPages() {
	...
	return Promise.all(pages.map((template) => buildPageByTemplate(template))).then((buildPages) => {
		...
		console.log(built.map((b) => `${`✓`.green} ${b.cyan}`).join("\n"));
 ...
 * @returns {Promise<void>}
 */
exports.buildPage = async function (path) {
 ...
 * @returns {Promise<void>}
 */
exports.buildPackage = async function (package) {
	/* Builds *.pug files in the site directory */
 ...
 * @returns {Promise<void>}
 */
exports.buildPackages = async function packages() {
 ...
 * @returns {Promise<void>}
 */
exports.buildDocs = async function () {
	return getAllData().then(() =>
		Promise.all([
			buildAllPackages(),
			copySiteDependencies(),
