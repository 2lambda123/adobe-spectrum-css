...
const { processCSS } = require("@spectrum-css/component-builder-simple");
...
async function watch_Styles(file) {
	...
	const localProcessorPath = path.join(`${dirName}/index.js`);
	if (!fs.existsSync(localProcessorPath)) {
		console.log(`${"✗".red}  No processing function found for ${relativePrint(localProcessorPath)}`);
        return copyTask();
	}

	if (fs.existsSync(outputDir)) await rimraf(outputDir);
	await require(localProcessorPath);
