...
(function () {
	function setFocus(textfield, input, focused) {
		var focusClass = input.classList.contains("is-keyboardFocused") ? "is-keyboardFocused" : "is-focused";
		if (focused) {
...
(function () {
	...
	window.addEventListener("keydown", function (event) {
		...
		if (menuItem) {
			...
			var closestPicker = event.target.closest(".spectrum-Picker");
			if (closestPicker) toggleOpen(closestPicker, true);
		}
	});
	...
	function getPickerForMenu(menuOrMenuItem) {
		...
		return popover && popover.previousElementSibling && popover.previousElementSibling.matches(".spectrum-Picker")
			? popover.previousElementSibling
			: null;
	}

	function getPopoverForPicker(picker) {
		return picker && picker.nextElementSibling && picker.nextElementSibling.matches(".spectrum-Popover")
	...
	window.addEventListener("click", function (event) {
		...
		if (picker) {
			...
			picker =
				popover.previousElementSibling && popover.previousElementSibling.matches(".spectrum-Picker")
					? popover.previousElementSibling
					: null;
...
function toggleSpectrumPopover(popover) {
	popover.classList.contains("is-open") ? popover.classList.remove("is-open") : popover.classList.add("is-open");
}
