export default class Slider {
    ...
    get label() {
        return this.el.querySelector(".spectrum-Slider-label");
    }

    get fill() {
        return this.el.querySelector(".spectrum-Slider-fill");
    }

    get valueContainer() {
        return this.el.querySelector(".spectrum-Slider-value");
    }

    get controls() {
        return this.el.querySelector(".spectrum-Slider-controls");
    }

    get handles() {
        return [...this.el.querySelectorAll(".spectrum-Slider-handle")];
    }

    get tracks() {
        return [...this.el.querySelectorAll(".spectrum-Slider-track")];
    }

    get ticks() {
        return [...this.el.querySelectorAll(".spectrum-Slider--tick")];
    }

    get buffers() {
        return [...this.el.querySelectorAll(".spectrum-Slider-buffer")];
    }
        ...
        if (this.activeHandle) {
            ...
            if (middleTrack) {
                if (this.isRTL) {
                    ...
                    if (rightHandle) middleTrack.style.left = 100 - parseFloat(rightHandle.style.right) + "%";
                } else {
                    if (leftHandle) middleTrack.style.left = leftHandle.style.left;
                    if (rightHandle) middleTrack.style.right = 100 - parseFloat(rightHandle.style.left) + "%";
                }
            }
        } else {
            if (!this.isColor) {
    ...
    calculateSliderPercent(mouseX) {
        ...
        const sliderOffsetLeft = this.el.offsetLeft + this.el.offsetParent.offsetLeft;

        const x = Math.max(Math.min(mouseX - sliderOffsetLeft, sliderOffsetWidth), 0);
    ...
    checkTrackStyles(track) {
        ...
        if (this.isRTL) track.style.background = track.style.background.replace("right", "left");
        else track.style.background = track.style.background.replace("left", "right");
    }
    ...
    updateBufferPosition(handle, percent) {
        ...
        if (percent >= bufferedAmount) {
            ...
            if (this.isRTL) {
                ...
                handlePosition = handle.parentElement.offsetWidth - handle.offsetLeft + handle.offsetWidth / 2;
            } else {
