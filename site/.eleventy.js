...
module.exports = (config) => {
    ...
    config.addCollection("packages", () => {
        return components.map((c) => ({
            name: c,
            stylesheet: existsSync(`${componentDir}/${c}/dist/index-base.css`)
                ? `index-base.css`
                : existsSync(`${componentDir}/${c}/dist/index.css`)
                ? `index.css`
                : null,
            folder: `/components/${c}`,
        }));
    ...
    config.addPassthroughCopy({
        ...
        "../assets/favicons": "/",
        // Pass through the storybook build for the production build
        ...(!isDev && {
            "../storybook/dist": "preview",
        }),
        // Pass through the tokens
        ...["@spectrum-css/vars", "@spectrum-css/expressvars"].forEach((t) => ({
            [`../node_modules/${t}/dist/spectrum-*.css`]: `dependencies/${t}`,
        })),
        "../tokens/dist": "dependencies/@spectrum-css/tokens",
    ...
    return {
        ...
        templateFormats: ["pug", "11ty.js", "md", "js", "html", "png", "svg", "ico"], // "njk",
