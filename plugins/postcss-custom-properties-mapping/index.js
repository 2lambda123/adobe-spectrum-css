...
/**
 * @typedef Options
 * @property {Map<string, string>} staticVars - a map of static variables to use for the component
 * @property {Map<string, string>} allVars - a map of all possible variables to use for the component
 */
...
module.exports = ({
	...
	OnceExit(root) {
		root.walkRules((rule) => {
			...
			rule.walkDecls(/^--/, (decl) => {
				internalVars.set(decl.prop, decl.value);
			});
			...
			root.walkDecls((decl) => {
				...
				value.walk((node) => {
					...
					// If the value is defined internally, no need to inject the fallback
					if (internalVars.has(v)) return;
				...
				decl.value = valueParser.stringify(value);
			});
		});
	},
});
