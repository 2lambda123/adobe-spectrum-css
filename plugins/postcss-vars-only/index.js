...
module.exports = function ({ rootClass, verbose = true, globalVars = new Map(), ignoreList = [] } = {}) {
	return {
				...
				Declaration(decl) {
					...
					parsed.walk((node) => {
						...
						if (!newValue || typeof newValue === "undefined" || newValue.includes(varName)) {
							if (verbose) {
								// If we still don't have a value, log a warning
								result.warn(`Undefined variable ${varName} used`, {
									node: decl,
									source: decl.source,
								});
							}
